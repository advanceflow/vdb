...
class Session:
    def __init__(self):
        ...
        if settings.DATASET == "WIKI":
            self.train_dataset = datasets.WIKI(
                root=settings.DATA_DIR,
                train=True,
                transform=datasets.wiki_train_transform,
            )
            self.test_dataset = datasets.WIKI(
                root=settings.DATA_DIR,
                train=False,
                transform=datasets.wiki_test_transform,
            )
            self.database_dataset = datasets.WIKI(
                root=settings.DATA_DIR,
                train=True,
                transform=datasets.wiki_test_transform,
            )
        ...
        if settings.DATASET == "MIRFlickr":
            ...
            self.test_dataset = datasets.MIRFlickr(
                train=False, database=False, transform=datasets.mir_test_transform
            )
            self.database_dataset = datasets.MIRFlickr(
                train=False, database=True, transform=datasets.mir_test_transform
            )
        ...
        if settings.DATASET == "NUSWIDE":
            ...
            self.test_dataset = datasets.NUSWIDE(
                train=False, database=False, transform=datasets.nus_test_transform
            )
            self.database_dataset = datasets.NUSWIDE(
                train=False, database=True, transform=datasets.nus_test_transform
            )
        ...
        if settings.DATASET == "WIKI":
            self.opt_I = torch.optim.SGD(
                [
                    {
                        "params": self.CodeNet_I.fc_encode.parameters(),
                        "lr": settings.LR_IMG,
                    },
                    {
        ...
        if settings.DATASET == "MIRFlickr" or settings.DATASET == "NUSWIDE":
            self.opt_I = torch.optim.SGD(
                self.CodeNet_I.parameters(),
                lr=settings.LR_IMG,
                momentum=settings.MOMENTUM,
                weight_decay=settings.WEIGHT_DECAY,
            )

        self.opt_T = torch.optim.SGD(
            self.CodeNet_T.parameters(),
            lr=settings.LR_TXT,
            momentum=settings.MOMENTUM,
            weight_decay=settings.WEIGHT_DECAY,
        )
    ...
    def train(self, epoch):
        ...
        self.logger.info(
            "Epoch [%d/%d], alpha for ImgNet: %.3f, alpha for TxtNet: %.3f"
            % (
                epoch + 1,
                settings.NUM_EPOCH,
                self.CodeNet_I.alpha,
                self.CodeNet_T.alpha,
            )
        )
        ...
        for idx, (img, F_T, labels, _) in enumerate(self.train_loader):
            ...
            S = (1 - settings.ETA) * S_tilde + settings.ETA * S_tilde.mm(
                S_tilde
            ) / settings.BATCH_SIZE
            S = S * settings.MU
            ...
            if (idx + 1) % (
                len(self.train_dataset) // settings.BATCH_SIZE / settings.EPOCH_INTERVAL
            ) == 0:
                self.logger.info(
    ...
    def eval(self):
        ...
        if settings.DATASET == "WIKI":
            re_BI, re_BT, re_L, qu_BI, qu_BT, qu_L = compress_wiki(
    ...
    def load_checkpoints(self, file_name="latest.pth"):
        ...
        self.logger.info(
            "********** The loaded model has been trained for %d epochs.*********"
            % obj["step"]
        )
